  ...
  return (
    <Layout>
      ...
      <div style={{ marginBottom:"24px" }} className="fade-up">
        <p style={{ color:"var(--accent)", fontSize:"12px", fontWeight:"600", letterSpacing:"0.1em", textTransform:"uppercase", marginBottom:"6px" }}>Overview</p>
        <h1 style={{ fontSize:"clamp(22px,6vw,36px)", color:"var(--text)", marginBottom:"4px" }}>
          {name ? `Hello, ${name.split(" ")[0]} 👋` : "Dashboard"}
        </h1>
        <p style={{ color:"var(--muted)", fontSize:"14px" }}>Live commodity prices across Tamil Nadu</p>
      </div>

      {/* My Stats — 3 cols even on mobile (small numbers = OK) */}
      {!loading && (
        <div style={{ display:"grid", gridTemplateColumns:"repeat(3,1fr)", gap:"10px", marginBottom:"20px" }}>
          {[
            { label:"Submitted", value:myPrices.length, color:"#6366f1", icon:"📋", link:"/my" },
            { label:"Approved",  value:approved,        color:"#10b981", icon:"✓",  link:"/my" },
            { label:"Unread",    value:unread,           color:"#f59e0b", icon:"🔔", link:"/notifications" },
            ...
            <Link key={label} to={link} style={{ textDecoration:"none" }}>
              <div className={`card fade-up fade-up-${i+1}`} style={{ borderTop:`2px solid ${color}`, cursor:"pointer", padding:"12px 10px", textAlign:"center" }}>
                ...
                <p style={{ color:"var(--muted)", fontSize:"11px", marginTop:"4px" }}>{label}</p>
              </div>
            </Link>
          ))}
        </div>
      ...
      {/* Platform stats — 2 cols on mobile */}
      <div style={{ display:"grid", gridTemplateColumns:"repeat(auto-fit,minmax(140px,1fr))", gap:"10px", marginBottom:"20px" }}>
        {[
          { label:"Commodities", value:"12", delta:"+2 this week",  color:"#f59e0b", icon:"🌾" },
          { label:"Markets",     value:"25", delta:"+3 this month", color:"#10b981", icon:"🏪" },
          { label:"Updates",     value:"48", delta:"Today",         color:"#6366f1", icon:"📊" },
        ].map(({label,value,delta,color,icon},i) => (
          <div key={label} className={`card fade-up fade-up-${i+4}`} style={{ padding:"14px" }}>
            ...
            <p style={{ color:"var(--muted)", fontSize:"11px", marginBottom:"2px" }}>{label}</p>
      ...
      <div className="card fade-up" style={{ marginBottom:"16px", padding:"16px" }}>
        <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:"14px" }}>
          <h3 style={{ fontSize:"16px", color:"var(--text)" }}>Recent Submissions</h3>
        ...
        {loading ? (
          ...
          <div style={{ textAlign:"center", padding:"20px 0" }}>
            <p style={{ color:"var(--muted)", fontSize:"14px", marginBottom:"12px" }}>No submissions yet.</p>
            <Link to="/add"><button className="btn">Add your first →</button></Link>
          </div>
        ) : myPrices.map(item => {
          ...
          return (
            <div key={item._id} style={{ display:"flex", alignItems:"center", gap:"10px", padding:"10px 0", borderBottom:"1px solid var(--border)" }}>
              ...
              <div style={{ flex:1, minWidth:0 }}>
                <p style={{ fontSize:"13px", fontWeight:"500", color:"var(--text)", overflow:"hidden", textOverflow:"ellipsis", whiteSpace:"nowrap" }}>{item.commodity}</p>
              ...
              <span style={{ fontSize:"10px", fontWeight:"600", padding:"2px 7px", borderRadius:"99px", background:`${sc}18`, color:sc, border:`1px solid ${sc}30`, textTransform:"capitalize", flexShrink:0 }}>{item.status}</span>
            </div>
          );
        })}
      </div>
      ...
      <div className="card fade-up" style={{ background:"linear-gradient(135deg,rgba(245,158,11,0.08),rgba(99,102,241,0.08))", border:"1px solid rgba(245,158,11,0.2)", padding:"16px" }}>
        <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", gap:"12px", flexWrap:"wrap" }}>
          <div>
            <h3 style={{ fontSize:"16px", color:"var(--text)", marginBottom:"3px" }}>Contribute today</h3>
            <p style={{ fontSize:"12px", color:"var(--muted)" }}>Help farmers with accurate market prices</p>
          </div>
          <Link to="/add"><button className="btn">Add Price →</button></Link>
        </div>
      </div>
    </Layout>
  );
