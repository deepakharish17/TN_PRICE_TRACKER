  ...
  return (
    <Layout>
      <div style={{ marginBottom:"20px" }} className="fade-up">
        <p style={{ color:"var(--accent)", fontSize:"12px", fontWeight:"600", letterSpacing:"0.1em", textTransform:"uppercase", marginBottom:"6px" }}>{t("liveBoard")}</p>
        <h1 style={{ fontSize:"clamp(22px,5vw,36px)", color:"var(--text)", marginBottom:"4px" }}>{t("marketPrices")}</h1>
        <p style={{ color:"var(--muted)", fontSize:"14px" }}>{t("latestPrices")}</p>
      </div>
        ...
        <div style={{ display:"flex", flexDirection:"column", gap:"8px" }}>
          <div style={{ display:"flex", gap:"8px", flexWrap:"wrap" }}>
            ...
            <select value={commodity} onChange={e=>setCommodity(e.target.value)}
              ...
              {commodities.map(c=><option key={c} value={c}>{cn(c)}</option>)}
            </select>
          </div>
          <div style={{ display:"flex", gap:"8px", alignItems:"center" }}>
            <div style={{ display:"flex", gap:"3px", background:"var(--surface2)", border:"1px solid var(--border)", borderRadius:"8px", padding:"3px" }}>
              {["cards","table"].map(v=>(
                ...
                }}>{v==="cards" ? t("cards") : t("table")}</button>
              ))}
            </div>
            {(district||commodity) && (
              <button className="btn-ghost" style={{ padding:"7px 12px", fontSize:"12px" }} onClick={()=>{setDistrict("");setCommodity("");}}>
                {t("clearFilters")}
              </button>
            )}
      ...
      {loading ? (
        <div style={{ textAlign:"center", padding:"60px", color:"var(--muted)" }}>{t("loading")}</div>
      ) : filtered.length===0 ? (
        <div className="card" style={{ textAlign:"center", padding:"48px" }}>
          <p style={{ fontSize:"32px", marginBottom:"10px" }}>🏪</p>
          <p style={{ color:"var(--text)", fontWeight:"600" }}>{t("noMarketPrices")}</p>
          <p style={{ color:"var(--muted)", fontSize:"13px", marginTop:"4px" }}>{t("noMarketPricesDesc")}</p>
        </div>
      ) : view==="cards" ? (
        <div style={{ display:"grid", gridTemplateColumns:"repeat(auto-fill,minmax(160px,1fr))", gap:"10px" }}>
          {Object.entries(grouped).map(([commodity,entries],i) => (
            <div key={commodity} className={`card fade-up fade-up-${Math.min(i%5+1,5)}`} style={{ padding:"14px" }}>
              <div style={{ display:"flex", alignItems:"center", gap:"8px", marginBottom:"10px" }}>
                ...
                <div style={{ flex:1, minWidth:0 }}>
                  <div style={{ display:"flex", alignItems:"center", gap:"4px", flexWrap:"wrap" }}>
                    <p style={{ fontWeight:"600", color:"var(--text)", fontSize:"13px", overflow:"hidden", textOverflow:"ellipsis", whiteSpace:"nowrap" }}>{cn(commodity)}</p>
                    {entries.some(e=>e.source==="scraped") && (
                      <span style={{ fontSize:"9px", background:"#22c55e22", color:"#22c55e", padding:"1px 5px", borderRadius:"4px", fontWeight:"600", flexShrink:0 }}>
                        {t("official")}
                      </span>
                    )}
                  </div>
              ...
              <p style={{ fontSize:"10px", color:"var(--muted)", marginBottom:"8px" }}>{t("avgKg")}</p>
              <div style={{ borderTop:"1px solid var(--border)", paddingTop:"8px" }}>
                {entries.slice(0,2).map(e=>(
                  <div key={e._id} style={{ display:"flex", justifyContent:"space-between", marginBottom:"4px" }}>
                    <p style={{ fontSize:"11px", color:"var(--muted)", overflow:"hidden", textOverflow:"ellipsis", flex:1 }}>{dn(e.district)}</p>
                  ...
                  <tr key={p._id}>
                    <td>
                      <div style={{ display:"flex", alignItems:"center", gap:"8px" }}>
                        <span style={{ fontSize:"16px" }}>{getIcon(p.commodity)}</span>
                        <span style={{ fontWeight:"500", color:"var(--text)", fontSize:"13px" }}>{cn(p.commodity)}</span>
                        {p.source==="scraped" && (
                          <span style={{ fontSize:"9px", background:"#22c55e22", color:"#22c55e", padding:"1px 5px", borderRadius:"4px", fontWeight:"600" }}>
                            {t("official")}
                          </span>
                        )}
                      </div>
                    ...
                    <td style={{ color:"var(--muted)", fontSize:"12px" }}>{dn(p.district)}</td>
